...
export function PayrollWorkspace({ page }: { page: PayrollPage }) {
  ...
  return <PageShell><div className="space-y-5">
    ...
    {page === "monthly" ? <Card className="border-border/80 bg-muted/20 shadow-none"><CardContent className="p-4 text-sm text-muted-foreground"><strong className="text-foreground">{t("الصافي = الأجر الخام + المكافآت + التسويات الإيجابية − التسبيقات − الاقتطاعات")}</strong><p className="mt-1 text-xs">{t("بالقطعة: الكميات المقبولة فقط × التعريفة المسجلة · باليوم: أيام العمل المعتمدة · بالشهر: الراتب الثابت للفترة")}</p></CardContent></Card> : null}
    {page === "payslip" ? <Card className="border-border/80 bg-muted/20 shadow-none"><CardContent className="flex items-center gap-2 p-4 text-xs text-muted-foreground"><Users className="size-4" />{t("الاطلاع والطباعة متاحان للمستخدمين المصرح لهم فقط")}</CardContent></Card> : null}
  </div></PageShell>;
}